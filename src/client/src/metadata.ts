...
export class Client {
     ...
     * @return OK
     */
    getAllResumeByUserId(userId: string | undefined): Promise<void> {
        let url_ = this.baseUrl + "/api/Resume/GetAllResumeByUserId?";
        if (userId === null)
        ...
        return this.http.fetch(url_, options_).then((_response: Response) => {
            return this.processGetAllResumeByUserId(_response);
        });
    }

    protected processGetAllResumeByUserId(response: Response): Promise<void> {
        const status = response.status;
        let _headers: any = {}; if (response.headers && response.headers.forEach) { response.headers.forEach((v: any, k: any) => _headers[k] = v); };
        if (status === 200) {
            return response.text().then((_responseText) => {
    ...
    /**
     * @param resumeId (optional) 
     * @return OK
     */
    getResumeByResumeId(resumeId: string | undefined): Promise<void> {
        let url_ = this.baseUrl + "/api/Resume/GetResumeByResumeId?";
        if (resumeId === null)
            throw new Error("The parameter 'resumeId' cannot be null.");
        else if (resumeId !== undefined)
            url_ += "resumeId=" + encodeURIComponent("" + resumeId) + "&";
        url_ = url_.replace(/[?&]$/, "");
        ...
        return this.http.fetch(url_, options_).then((_response: Response) => {
            return this.processGetResumeByResumeId(_response);
        });
    }

    protected processGetResumeByResumeId(response: Response): Promise<void> {
        const status = response.status;
        let _headers: any = {}; if (response.headers && response.headers.forEach) { response.headers.forEach((v: any, k: any) => _headers[k] = v); };
        if (status === 200) {
            return response.text().then((_responseText) => {
